package com.alex.imagedemo.utils;
...
public class HistogramImg {
    ...
    public static int[] getHistogram(BufferedImage bi) {
        ...
        for (int i = 0; i < bi.getWidth(); i++) {
            for (int j = 0; j < bi.getHeight(); j++) {
                ...
                int red = getRegion(PicUtil.getRed(rgb));
                int green = getRegion(PicUtil.getGreen(rgb));
                int blue = getRegion(PicUtil.getBlue(rgb));
    ...
    public static double calculate(int[] histogram1, int[] histogram2) {
        //x1*x2 + y1*y2 + z1*z2 + .....
        //(x1^2 + y1^2 + z1^2 + .....)^-2 * (x2^2 + y2^2 + z2^2 + .....)^-2
